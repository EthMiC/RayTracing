function Color(r, g, b, calculation){
    let min_clr = calculation ? 0 : 20;
    return "rgb(" + Math.max(r, min_clr) + ", " + Math.max(g, min_clr) + ", " + Math.max(b, min_clr) + ")";
}
...
function ColorAdd(_clr1, _clr2){
    ...
    return Color((_clr1[0] + _clr2[0]), (_clr1[1] + _clr2[1]), (_clr1[2] + _clr2[2]), true);
}
...
function ColorMult(_clr1, _clr2){
    ...
    _clr1 = GetRGBValues(_clr1);
    _clr2 = GetRGBValues(_clr2);
    let _clr = Color(_clr1[0] * _clr2[0], _clr1[1] * _clr2[1], _clr1[2] * _clr2[2], true);
...
function ColorScalarMult(_clr, scalar){
    ...
    return Color((_clr[0] * scalar), (_clr[1] * scalar), (_clr[2] * scalar), true);
}
...
function ColorLerp(_clr1, _clr2, _value){
    ...
    let clr = Color(lerp(clr1RGB[0], clr2RGB[0], _value), lerp(clr1RGB[1], clr2RGB[1], _value), lerp(clr1RGB[2], clr2RGB[2], _value), true);
...
function scaleLight(_clr) {
    ...
    return Color(_clr[0], _clr[1], _clr[2], true);
}